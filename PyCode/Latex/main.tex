...
\begin{figure}[!htb]
  \centering
  \begin{subfigure}[b]{0.9\linewidth}
    \includegraphics[width=\linewidth]{run3_route.png}
...
To make better use of the acquired data, amateur runners can use the
Heart Rate Efficiency (HRE) metric, defined as the quotient of velocity
($v$) and heart rate (HR):
...
alone \cite{Votyakov2025}; we note that this specific formulation is
currently supported only by a preprint and would benefit from
peer-reviewed replication before wider adoption. Geospatial tracking
performed well overall in our data, with a study-wide mean spatial
offset of $\approx$4.56\,m across all six runs (range 2.1--8.3\,m
per-run mean, max.\ 30.5\,m; Figure~\ref{fig:gpsoffset}), though this
combined average should not obscure the per-run variability visible in
